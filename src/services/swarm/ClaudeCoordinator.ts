...
export class ClaudeCoordinator {
  async hasClaudeKey(): Promise<boolean> {
    try {
      const key = await invoke<string>('get_api_key', { provider: 'claude' });
      return Boolean(key);
    } catch {
      return false;
    }
  ...
  async createLaunchPlan(goal: string, workspacePath: string, roles: AgentRoleType[]): Promise<CoordinatorPlan> {
    ...
    const raw = await invoke<string>('ask_claude', { prompt });
  ...
  async createSyncResult(
    ...
    const raw = await invoke<string>('ask_claude', {
      prompt,
    });
